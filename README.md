...
# 7. System Architecture (Mermaid)

```mermaid
flowchart TD
  FE[Frontend App] -->|JWT| API[FastAPI Service]
  API -->|SELECT| DB[(PostgreSQL)]
  API -->|Redis| RL[Rate Limiter]
  API -->|HTTP| Gebeta[Gebeta Maps ONM]
  API -->|HTTP| Gemini[Google Gemini]
  subgraph AI Pipeline
    G1[Geocode Step]-->S1[Search Step]
    S1-->T1[Transport Cost Step]
    T1-->R1[Rank Step]
    R1-->E1[Reason Step]
  end
  API --> AI Pipeline
  AI Pipeline --> API
  API --> Logs[(RecommendationLogs)]
```

Key points:
- The service queries `properties` directly (APPROVED only) with multiple fallbacks.
- Transport costs use Gebeta ONM; sanitized coordinates and matrix parsing.
- Reasons are generated by Gemini using a prompt-engineered, multilingual template.
- Preferences and logs are persisted; base property data is read-only from this service.

---

# 8. Database ER Diagram (Mermaid)

```mermaid
erDiagram
  users ||--o{ TenantPreferences : has
  TenantPreferences ||--o{ RecommendationLogs : logs
  users {
    UUID id PK
    string email
  }
  TenantPreferences {
    int id PK
    UUID user_id FK
    string job_school_location
    float salary
    string house_type
    int family_size
    string[] preferred_amenities
    datetime created_at
  }
  RecommendationLogs {
    int id PK
    int tenant_preference_id FK
...
# 9. API Quick Start (Frontend)

See `API_DOCUMENTATION.md` for full details. Minimal usage:
...
- `GEBETA_API_KEY` — Gebeta Maps key
- `GEMINI_API_KEY` — Gemini key
- `REDIS_URL` — Redis for rate limiting
- `READ_ONLY_MODE` — if used, applies to base data policies (recommendation logs still saved)

Security best practices:
- Use a least-privileged DB role: SELECT-only on base tables (e.g., `properties`), INSERT on logs/preferences.
- Never commit real secrets; keep `.env` gitignored.
- Enable TLS/SSL to DB (already configured in `app/database.py`).
- Restrict CORS origins in `app/main.py` to your production domains.
...
1) Build the image
```bash
docker build -t ai-recommendation:latest .
```

2) Run with env file and port mapping
```bash
docker run --env-file .env -p 8005:8005 ai-recommendation:latest
```

## Render/Vercel/Other

- Ensure `.env` variables are set in the platform’s secrets panel.
- Configure health check path `/health`.
- Ensure outbound access to Gebeta and Gemini.

## Observability

- Structured logs via `structlog` (`app/core/logging.py`).
- Add your APM/metrics as needed.

---

# 12. Operational Notes

- Rate limiting: `POST /api/v1/recommendations` is capped at 5/min/user.
- Fallbacks ensure results even when strict filters fail (returns APPROVED only).
- Reason generation: numbered, multilingual; prompt in `app/services/promttemplet.py`.
- ONM: follows redirects; supports matrix schema (`origin_to_destination`) and legacy `directions`.

---

# 13. Contributing (Open Source Ready)

- Code style: PEP8; black/ruff recommended.
- Issues/PRs: include reproduction steps and logs.
- Tests: `tests/test_recommendation.py` with pytest.

---

For detailed endpoint reference and frontend snippets, see `API_DOCUMENTATION.md`.
